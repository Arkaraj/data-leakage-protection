...
def checkdocument(request):
    ...
    if request.method=='POST':
        ...
        if p==0:
        	...
        	messages.error(request, 'aryan leaked')
        elif p==2:
        	messages.error(request, 'shivam leaked')
        elif p==3:
        	messages.error(request, 'piyush leaked')
        else:
...
def history(request):
	...
	q = History.objects.all().order_by("-leakdetetcetedat")
	context = {
		'nbar': 'history',
		'data': q,
	...
	if request.method == 'POST':
		if request.POST.get('filename'): #filename is name attribute of the button clicked in template
			...
			if(designation<int(delfile.uploadlevel) ):
				...
				a.save()
				print("log 2")
				to=['arkaraj2017@gmail.com']
				b = username + " tried to delete document "+name 
				email_id='pratyaypiyush2001@gmail.com'
				reciever_id="arkaraj2017@gmail.com"
				# email_pass=input("Type your password and press enter:")
				email_pass="emscfnefsvvnveoz"
				print("log 3")
				msg=EmailMessage()
				...
				with smtplib.SMTP_SSL('smtp.gmail.com',port)as smtp:
					...
					# smtp.sendmail(email_id, reciever_id, msg)
